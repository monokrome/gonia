...
import { processElementTree, PROCESSED_ATTR } from '../src/process.js';
...
import { clearElementScopes, clearRootScope, getElementScope } from '../src/scope.js';
...
describe.each([
  ['client', Mode.CLIENT],
  ['server', Mode.SERVER],
] as const)('processElementTree (%s)', (_label, mode) => {
  ...
  describe('scope handling', () => {
    it('should use existing scope when provided', () => {
      ...
      processElementTree(element, parentState, mode, { existingScope });

      expect(element.textContent).toBe('local');
      expect(getElementScope(element)).toBe(existingScope);
    });
    ...
    it('should create child scope with additions', () => {
      ...
      processElementTree(element, parentState, mode, {
        scopeAdditions: { itemValue: 'item' }
      });

      expect(element.textContent).toBe('item');
      expect(getElementScope(element)).not.toBe(parentState);
    });
  ...
  describe('recursive processing', () => {
    ...
    it('should process deeply nested elements', () => {
      ...
      processElementTree(root, state, mode);
  ...
  describe('scope inheritance in tree', () => {
    it('should pass existing scope to root only', () => {
      ...
      processElementTree(root, parentState, mode, { existingScope });
  ...
  describe('scope persistence across re-renders', () => {
    it('should maintain scope identity when reusing existingScope', () => {
      ...
      processElementTree(element1, parentState, mode, {
        existingScope: persistentScope
      });

      expect(element1.textContent).toBe('0');

      const scope1 = getElementScope(element1);

      const element2 = document.createElement('span');
      element2.setAttribute('g-text', 'count');
      processElementTree(element2, parentState, mode, {
        existingScope: persistentScope
      });

      const scope2 = getElementScope(element2);
  ...
  describe('nested structural directives', () => {
    afterEach(() => {
      clearElementScopes();
      clearRootScope();
    });
